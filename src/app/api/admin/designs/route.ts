...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      pending: allDesigns?.filter(d => !d.is_approved && d.is_public).length || 0,
      rejected: allDesigns?.filter(d => !d.is_approved && !d.is_public).length || 0,
    };
    ...
    if (status === 'pending') {
      filteredDesigns = filteredDesigns.filter(d => !d.is_approved && d.is_public);
